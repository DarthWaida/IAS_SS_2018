...
def formatSensorData(clientID, visionSensor):
    ...
    sensorPosition = vrep.simxGetObjectPosition(clientID, visionSensor, getObjectBase(clientID, 'Plane'), vrep.simx_opmode_oneshot_wait)[1][:]
    sensorOrientation = vrep.simxGetObjectOrientation(clientID, visionSensor, getObjectBase(clientID, 'Plane'), vrep.simx_opmode_oneshot_wait)
...
def moveToGoalLine(clientID, wheelJoints, sensorHandles, goalLine):
    ...
    linePos = getPointOnGoalLine(goalLine, robotPosition)
...
def followWall(clientID, wheelJoints, visionSensors):
    ...
    #sensordata = getSensorData(clientID, visionSensors)

    while(True==True):
        print("stink")
        rotate(clientID, wheelJoints, 1)

    #middle = len(sensordata) // 2

#   IMPORTANT: set booleans right
    ...
    if clientID!=-1:
        ...
        while(not hasReachedPosition(getRobotPos(clientID), goalPosition)):
            if(not onWall and onLine):
                ...
                proceedToPoint(clientID,wheelJoints, sensorHandles, goalPosition[0], goalPosition[1], getAngle(getRobotPos(clientID), goalPosition))
