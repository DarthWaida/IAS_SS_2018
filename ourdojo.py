...
def getAngleSin(x1, y1, x2, y2):
    ankat = x1 - x2
    gegkat = y1 - y2
    hyp = math.sqrt(np.square(ankat) + np.square(gegkat))
    return np.degrees(np.arcsin(gegkat/hyp))
...
def followWall(clientID, wheelJoints, visionSensors):
    ...
    while (onWall):
        ...
        angle2 = getAngleSin(roboPos[0], roboPos[1],5, 4)
        print('Winkel: ', roboPos[2])
        print('winkel sin: ', angle2)
        rotate(clientID, wheelJoints, angle2)
        [left, right, center] = getSensorData(clientID, visionSensors)
